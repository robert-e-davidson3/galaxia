...
const CELL_ON_COLOR: Color = Color::srgb(0.0, 0.0, 0.0);
const CELL_OFF_COLOR: Color = Color::srgb(1.0, 1.0, 1.0);
...
impl CellBundle {
    pub fn new(x: u8, y: u8, cols: u8, rows: u8) -> Self {
        ...
        Self {
            ...
            fill: Fill::color(CELL_OFF_COLOR),
        }
    }

    pub fn turn_on(entity: Entity, query: &mut Query<&mut Fill, With<Cell>>) {
        if let Ok(mut fill) = query.get_mut(entity) {
            fill.color = CELL_ON_COLOR;
        }
    }

    pub fn turn_off(entity: Entity, query: &mut Query<&mut Fill, With<Cell>>) {
        if let Ok(mut fill) = query.get_mut(entity) {
            fill.color = CELL_OFF_COLOR;
        }
    }
}
...
pub fn cell_update(
        ...
        cell_query.iter()
    {
        ...
        if CELL_AREA.is_within(
            ...
            CellBundle::turn_off(cell_entity, &mut fill_query);
            // Record extraction
